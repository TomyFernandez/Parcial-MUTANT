...
public class MutantControllerTest {
    ...
    @Test
    public void testCheckMutant_Exception() {
        String[] dna = {"ATGCGA", "CAGTGC", "TTATGT", "AGAAGG", "CCCCTA", "TCACTG"};
        MutantRequest mutantRequest = new MutantRequest();
        mutantRequest.setDna(dna);
        mutantRequest.setNombre("Nombre");
        ...
        Mockito.when(mutantService.isMutant(any(String[].class))).thenThrow(new RuntimeException("Error"));

        ResponseEntity<?> response = mutantController.checkMutant(mutantRequest);
        Assert.assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        Assert.assertEquals("{\"error\":\"Error, por favor intente más tarde\"}", response.getBody());
